# WaterAInalytics v0.10.0 — Release Validation

This document defines the final validation workflow for the `v0.10.0` release line.

## Goals

- verify that the documented release assets are present;
- verify that the key runtime modules still import cleanly;
- verify that the release manifest reflects the article-ready workflow;
- export a machine-readable JSON report for archival and audit;
- provide a repeatable operator procedure before tagging the consolidated `v0.10.0` release.
...
1. Run the targeted pytest suite used during the `v0.10.0` article-ready steps.
...
5. Only then create the consolidated `v0.10.0` tag.
